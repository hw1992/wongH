...
public class ExcelExtractor {
  ...
  /**
   * 解析excel,默认第一行为表头
   * 数字在excel转bean的时候都会格式化成double类型
   * 需要修改此方法，将bean内某其他类型的字段排除
   *
   * @param type         excel类型
   * @param excel        excel输入流
   * @param tClass       要生成的实体类
   * @param headerMapper 对应解析表格
   * @param sheetIndex   第几张表
   * @return 解析后集合
   * @throws IllegalArgumentException 解析错误
   */
  public static <T> List<T> extractFrom(ExcelType type, InputStream excel, Class<T> tClass, Map<String, String> headerMapper, Integer sheetIndex) throws IllegalArgumentException, IOException {
    Workbook workbook;
    try {
